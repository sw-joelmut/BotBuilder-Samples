This sample demonstrates a simple echo bot with state. The bot maintains a simple counter that increases with each message from the user.
...
- In a terminal, navigate to the following directory:
  ```bash
  cd sample\javascript_nodejs\02.echobot-with-counter
  ```

  **Optional:** Update the `.env` file under `sample\javascript_nodejs\02.echobot-with-counter` with your **botFileSecret**.
