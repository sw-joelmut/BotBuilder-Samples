...
const samplesFolder = path.resolve(`C:/repos/BotBuilder-Samples/experimental`);

const bots = [
  {
    name: "0-adaptive-dialog",
    folder: "adaptive-dialog/javascript_nodejs/08.todo-bot-with-luis-qnamaker",
  },
  {
...
for (const template of templates) {
  ...
  const logsPath = path.resolve(
    path.join(__dirname, `/logs/${template.name}/experimental-${now}.log`)
  );
