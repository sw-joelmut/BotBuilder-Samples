...
(async () => {
  ...
  const deploymentStatus = await bottester.deploy();
  const connectionStatus = await bottester.status();
...
interface BotTesterOptions extends BotDeployment {}
...
class BotDeployment {
  ...
  constructor(private options: BotDeployment) {}
